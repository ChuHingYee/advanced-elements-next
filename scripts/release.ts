...
const buildFile = async (packageName: string) => {
...
const getNewVersion = (version: string, commits: Commit[]) => {
  ...
  if (intersection.length) {
    return semver.inc(
      version,
      intersection[0].substring(2) as semver.ReleaseType
    )
  }
...
;(async () => {
  try {
    ...
    const customVersion = argv.version
    if (!packageName) {
    ...
    const { default: pkg }: RepoPackage = await import(
    ...
    if (customVersion) {
      ...
      newVersion = getNewVersion(pkg.version, commits) as string
    }
